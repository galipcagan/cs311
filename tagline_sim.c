...
// Defines
#define TLINE_ARGUMENTS "hvful:"
#define USAGE \
	"USAGE: tagline_sim [-h] [-v] [-l <logfile>] [-f] <workload-file>\n" \
	"\n" \
	"where:\n" \
...
int main(int argc, char *argv[]) {
	...
	// Local variables
	int ch, unit_tests = -0, log_initialized = 0;
			...
			if (sscanf(line, "%s %hu %hu %u %s", command, &tagnum, &num_blocks, &blocknum, text) != 5) {
				...
				if (strncmp(command, "INIT", 5) == 0) {
					...
					if (disk_failures) {
						...
						if (raid_fail_disk((RAIDDiskID)tagnum) || (raid_disk_signal())) {
